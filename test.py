...
def gpt4_json_convert(text):
    
    client = OpenAI()

    prompt = f" don't give any explantion. please analyze and convert file data from this {text} into  json template : {resume_temp}  and in response please return only json file"
...
def main():
    ...
    if(resume!=None):
        ...
        print(resume)
        posting = convert_posting_json(posting)
        resume = convert_resume_json(resume)
        score_details = match_resume(posting,resume)
    ...
    #print(gpt4_json_convert(resume))
    resume = extract_text_from_pdf(pdfFile)
    resu = gpt4_json_convert(resume)
    print(resu)
